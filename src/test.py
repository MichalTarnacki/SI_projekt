...
def test_quantitative_with_wide_spectro(testfiles, modelfile, with_previous_state):
    clf = load(f'{macros.model_path}{modelfile}.joblib')
    scaler = load(f'{macros.model_path}{modelfile}_scaler.joblib')

    x_test, y_test, chunk_size = dataset.build_wide_spectro(testfiles, macros.test_path, with_previous_state)
    x_test_std = scaler.transform(x_test)
